...
static void result_fields_cb(void* context) {
    MonetaApp* app = context;
    view_dispatcher_send_custom_event(app->view_dispatcher, MonetaCustomEventShowFields);
}
...
void moneta_scene_result_on_enter(void* context) {
    ...
    card_view_set_fields_callback(app->card_view, result_fields_cb, app);
...
bool moneta_scene_result_on_event(void* context, SceneManagerEvent event) {
    ...
    if(event.type == SceneManagerEventTypeCustom) {
        switch(event.event) {
        case MonetaCustomEventShowFields:
