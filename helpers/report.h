 ...
 *
 * The full card number is displayed directly.
 */
...
static void draw_exposure_bar(Canvas* canvas, uint8_t score) {
    ...
    if(fill > 0) {
        canvas_draw_box(canvas, x + 2, y + 2, fill, h - 4);
    }
...
static void card_view_draw(Canvas* canvas, void* model) {
     ...
     *
     * Previously:
     *   revealed -> full number
     *   otherwise -> masked number
     *
     * Now the number is always displayed.
     */
    if(c->has_pan) {
    ...
    /*
     * No more "Hold OK: show number" hint because the PAN
     * is already permanently visible on this screen.
     */
    const char* hint;
...
static bool card_view_input(InputEvent* event, void* context) {
    ...
    if(event->key == InputKeyOk) {
        /*
         * Short press still opens "What leaked".
         *
         * Long press / release are no longer used to reveal/hide
         * the PAN because the PAN is always visible.
         */
        if(event->type == InputTypeShort) {
            if(view->fields_cb) {
                view->fields_cb(view->fields_ctx);
            }
...
CardView* card_view_alloc(void) {
     ...
     * It no longer controls PAN visibility.
     */
    with_view_model(
        view->view,
        CardViewModel * m,
        {
            m->reveal_allowed = true;
        },
        false);
...
void card_view_set_reveal_allowed(
    CardView* view,
    bool allowed) {
...
void card_view_set_fields_callback(
    CardView* view,
    CardViewCallback cb,
    void* context) {
...
void card_view_set_log_callback(
    CardView* view,
    CardViewCallback cb,
    void* context) {
